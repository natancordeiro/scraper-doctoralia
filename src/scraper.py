...
class DoctorScraper:
    ...
    def get_all_reviews(self, reviews: list, doctor_id: str, reviews_count: int) -> list:
        ...
        try:
            ...
            with tqdm(total=reviews_count, desc="Processando Reviews", unit=" Reviews") as pbar:
                # já conta as reviews coletadas da página 1 (perfil)
                pbar.update(len(reviews))

                while True:
                    # trava de segurança de 20 minutos
                    ...
                    if "html" not in data:
                        logger.warning("Resposta inesperada do endpoint de reviews (sem 'html').")
                        break

                    soup = BeautifulSoup(data["html"], "html.parser")

                    # Novo seletor robusto (usa atributos estáveis do schema.org + data-test-id)
                    review_blocks = soup.select('[data-test-id="opinion-block"][itemprop="review"]')
                    if not review_blocks:
                        # fallback se mudarem o data-test-id novamente, mas manterem schema.org
                        review_blocks = soup.select('[itemprop="review"]')

                    # Se não tiver blocos, acabou a paginação
                    if not review_blocks:
                        break

                    for review in review_blocks:
                        name_el    = review.select_one('[itemprop="author"] [itemprop="name"], [itemprop="name"]')
                        date_el    = review.select_one('time[itemprop="datePublished"], time')
                        comment_el = review.select_one('[itemprop="reviewBody"]')

                        reviews.append({
                            "Reviewer Name": name_el.get_text(strip=True) if name_el else None,
                            "Review Date": date_el.get_text(strip=True) if date_el else None,
                            "Review Comment": comment_el.get_text(separator=" ", strip=True) if comment_el else None,
                        })

                    # Atualiza a barra com o que realmente entrou
                    pbar.update(len(review_blocks))

                    # Se já atingimos o total reportado no perfil, podemos parar
                    if len(reviews) >= reviews_count:
                        break

                    # Heurística adicional: se a API informar numRows/limit e vier menor que o limit, acabou
                    if data.get("numRows") is not None and data.get("limit") is not None:
                        if data["numRows"] < data["limit"]:
                            break
